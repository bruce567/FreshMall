                	...
                	<div class="address_list_div">
		              <span>
		                ...
		                <c:if test="${buy.buy_status==1}">
							<p style="color:#ff00ff">订单状态：未支付</p>
						</c:if>
						<c:if test="${buy.buy_status==2}">
							<p style="color:#ff00ff">订单状态：等待发货</p>
						</c:if>
						<c:if test="${buy.buy_status==3}">
							<p style="color:#ff00ff">订单状态：等待确认收货</p>
						</c:if>
						<c:if test="${buy.buy_status==4}">
							<p style="color:#ff00ff">订单状态：待评价</p>
						</c:if>
						<c:if test="${buy.buy_status==5}">
							<p style="color:#ff00ff">订单状态：已完成</p>
						</c:if>
		                <div class="address_list_bottom">
		                  <!-- 未发货可以取消订单 -->
		                  <c:if test="${buy.buy_status==2}">
		                  	<button onclick="buyDelete('${buy.buy_id}')" class="addredd_a2 del" style="background-color:#ff7f50;width:90px;height:28px;border-radius:6px;border:0px;">
		                      <i class="iconfont icon-iconfontshanchu"></i>
		                        <a style="color:#FFFFFF;font-size:16px">取消订单</a>
		                    </button>
		                  </c:if>
		                  <!-- 已发货可以确认收货 -->
		                  <c:if test="${buy.buy_status==3}">
		                  ...
		                  <!-- 确认收货后可以评价     float:right   <i class="iconfont icon-iconfontshanchu"></i>  -->
		                  
<!--  		                  <c:if test="${buy.buy_status==4}">
		                    
		                    <input type="submit" value="评价" href="frontuserevaluate?buy_id=${buy.buy_id}&goods_id=${buy.goods_id}" class="addredd_a2 del"
		                           style="background-color:#ff7f50;color:#ffffff;width:60px;height:30px;border:0px;">
		                    
		                  </c:if>    -->
		                  
		                  
		                  <c:if test="${buy.buy_status==4}">
		                    <button style="background-color:#ff7f50;width:80px;height:28px;border:0px;float:right;text-align:center;border-radius:6px;border:0px;">
		                  	 <a href="frontuserevaluate?buy_id=${buy.buy_id}&goods_id=${buy.goods_id}" class="addredd_a2 del" style="color:#FFFFFF;font-size:16px;">去评价</a>		   
		                    </button>   
		                  </c:if>  
